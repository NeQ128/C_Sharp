    ...
    public partial class Form1 : Form
    {
        ...
        */
        ...
        private string MyIP()
        {
            ...
            {
                if (ip.AddressFamily == AddressFamily.InterNetwork && ip.ToString().Substring(0,7) != "192.168")    //如果IP為「IPv4」格式
                {
                    //IP位置為"192.168"開頭的IP指某個區域網路內部提供的浮動IP
                    //如果使用這種IP當作伺服器IP，則只能供區域網路連線使用
        ...
        */
        ...
        {
            string L = "L";     //存放成員名單的變數，並先放入代表成員名單的指令碼(字頭)
            for (int i = 0;i < listBox1.Items.Count; i++)
            {
                L += listBox1.Items[i];         //將線上名單逐一加入成員名單的字串L
            ...
            while (true)
            {
                try
                {
                    ...
                    switch (Cmd)        //根據使用者的動作決定行為
                    {
                        ...
                        case "9":
                            SendAll(Msg);               //將訊息傳送給所有人
                            break;
                            
                        default:
                            string[] C = Str.Split('|');//分割收件者與訊息
                            SendTo(Cmd + C[0], C[1]);   //將 C[0]:訊息 傳給 C[1]:收件者
                            break;
                    }
                }
                catch
        ...
        {
            byte[] B = Encoding.Default.GetBytes(Str);      //將要傳送的訊息轉譯為純資料
            Socket Sck = (Socket)HT[User];                  //取出發送對象User的通訊物件
            Sck.Send(B, 0, B.Length, SocketFlags.None);     //傳送訊息資料
        }
        ...
        {
            byte[] B = Encoding.Default.GetBytes(Str);      //將要傳送的訊息轉譯為純資料
            foreach (Socket s in HT.Values)                 //取出HT雜湊表裡所有的值(Socket)
            {
